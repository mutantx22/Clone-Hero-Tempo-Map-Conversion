...
def snap_to_nearest_beat_or_half(tick, resolution):
    half_beat = resolution // 2
    return int(round(tick / half_beat) * half_beat)
...
def convert_notes(notes_text, old_bpm_events, new_bpm_events):
    ...
    for line in notes_text.splitlines():
        ...
        if m:
            ...
            # Snap to nearest beat or halfway point
            new_tick = snap_to_nearest_beat_or_half(new_tick, NEW_RESOLUTION)
            ...
            if length > 0:
                ...
                # Snap end independently
                new_end_tick = snap_to_nearest_beat_or_half(
                    new_end_tick, NEW_RESOLUTION
                )
                
                new_length = max(0, new_end_tick - new_tick)
            else:
                new_length = 0
